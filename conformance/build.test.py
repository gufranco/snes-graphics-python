...
class DefinitionTest(unittest.TestCase):
    ...
    def test_it_names_every_function_it_lifts(self) -> None:
        held = [one["gives"] for one in build.reference()["extracts"]]

        self.assertEqual(held, ["unpack_native_tile", "pack_native_color", "pack_native_mapentry"])

    def test_and_the_file_each_one_lives_in(self) -> None:
        for one in build.reference()["extracts"]:
            self.assertTrue(one["source"].endswith(".h"), one["gives"])

    def test_and_the_markers_that_bound_it_inside_that_file(self) -> None:
        for one in build.reference()["extracts"]:
            self.assertIn(one["gives"], one["from"])
            self.assertTrue(one["until"], one["gives"])
...
class LocalUpstream:
    ...
    def __init__(self) -> None:
        ...
        (self.where / "first.h").write_text(
            "before\ninline int one (void) { return 1; }\ninline int two (void) { return 2; }\n"
        )
        (self.where / "second.h").write_text(
            "before\ninline int three (void) { return 3; }\n} /* namespace */\n"
        )
        for step in (
            ...
            "extracts": [
                {
                    "source": "first.h",
                    "from": "inline int one",
                    "until": "inline int two",
                    "gives": "one",
                },
                {
                    "source": "second.h",
                    "from": "inline int three",
                    "until": "} /* namespace */",
                    "gives": "three",
                },
            ],
...
class ExtractTest(unittest.TestCase):
    def test_every_function_is_lifted_out_of_the_file_it_shares(self) -> None:
        upstream = LocalUpstream()
        ...
        self.assertIn("inline int one", held)
        self.assertIn("inline int three", held)

    def test_and_nothing_around_them_comes_with_it(self) -> None:
        upstream = LocalUpstream()
        into = Path(tempfile.mkdtemp()) / "bodies.inc"
        ...
        self.assertNotIn("inline int two", into.read_text())
    ...
    def test_a_source_whose_opening_marker_has_moved_is_reported(self) -> None:
        upstream = LocalUpstream()
        (upstream.where / "first.h").write_text("nothing the pin names\n")

        with self.assertRaises(build.Usage):
            build.extract(upstream.where, upstream.pinned(), Path("/tmp/x.inc"))

    def test_and_one_whose_closing_marker_has_moved_is_reported_too(self) -> None:
        """A slice that runs to the end of a file is worse than one that fails."""
        upstream = LocalUpstream()
        (upstream.where / "second.h").write_text("before\ninline int three (void) { return 3; }\n")
