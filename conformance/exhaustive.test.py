...
class CheckTest(unittest.TestCase):
    def test_every_check_reports_how_many_cases_it_settled(self):
    ...
    def test_every_check_says_what_it_settles(self):
        for check in exhaustive.CHECKS:
            self.assertTrue(check.summary.strip())

    def test_the_checks_between_them_settle_six_figures_of_cases(self):
...
class RunTest(unittest.TestCase):
    ...
    def test_naming_one_check_runs_only_that_one(self):
        self.assertEqual(exhaustive.run(["--only", exhaustive.CHECKS[0].name]), 0)

    def test_naming_a_check_that_does_not_exist_is_refused(self):
    ...
    def test_the_default_is_to_run_them_all(self):
...
class CatchesDefectsTest(unittest.TestCase):
    ...
    def broken(self, name, module, attribute, replacement):
    ...
    def test_a_tile_that_does_not_round_trip_is_caught(self):
        found = self.broken(
    ...
    def test_a_plane_reaching_the_wrong_bit_is_caught(self):
        found = self.broken(
    ...
    def test_a_colour_that_does_not_narrow_back_is_caught(self):
        found = self.broken("palette", exhaustive.palette, "to_word", lambda r, g, b: 0)

        self.assertFalse(found.clean)

    def test_a_map_entry_that_does_not_round_trip_is_caught(self):
        found = self.broken(
            "tilemap",
            exhaustive.tilemap,
            "Entry",
    ...
    def test_a_sprite_slot_read_from_the_wrong_place_is_caught(self):
        found = self.broken(
            "oam-high",
            exhaustive.oam,
            "decode",
    ...
    def test_a_sprite_slot_that_disturbs_a_neighbour_is_caught(self):
        found = self.broken(
            "oam-high",
            exhaustive.oam,
            "decode",
    ...
    def test_a_run_with_a_broken_check_reports_the_failure(self):
