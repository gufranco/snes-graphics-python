...
nothing to measure. A layout is right or wrong, and where the input space is
small enough it is walked from end to end rather than sampled: every two bit tile
that can exist, every colour the hardware can name, every map entry word. The
claim is not that this agrees with a reference on the cases somebody thought to
try. It is that there is no case left.

Where a space cannot be walked, because a four bit tile has more states than is
worth counting, what is walked instead is the property that actually matters:
each plane against each pixel, so a plane reaches its own bit of every pixel and
no other bit of anything.
