...
class ExamineTest(unittest.TestCase):
    ...
    def test_and_one_finding_per_layout_it_covers(self):
    ...
    def test_a_layout_that_will_not_decode_is_reported_rather_than_hidden(self):
    ...
    def test_a_layout_that_decodes_says_how_much_came_out(self):
        for one in doctor.examine():
            if one.name == "4bpp":
...
class RoundTripTest(unittest.TestCase):
    ...
    def test_the_report_says_whether_a_tile_survives_the_round_trip(self):
        self.assertIn("round trip", [one.name for one in doctor.examine()])

    def test_a_tile_that_survives_is_reported_as_well(self):
        for one in doctor.examine():
            if one.name == "round trip":
                self.assertTrue(one.ok)

    def test_one_that_comes_back_different_is_a_failure(self):
    ...
    def test_and_the_report_says_how_many_bytes_came_back(self):
        found = doctor._round_trip(lambda _name, data: data, lambda _name, _sheet: b"\x00")

        self.assertIn("1 byte", found.detail)

    def test_a_round_trip_that_throws_is_reported_rather_than_swallowed(self):
        def boom(_name, _data):
...
class ExhaustiveTest(unittest.TestCase):
    def test_the_report_names_what_is_settled_exhaustively(self):
        self.assertIn("settled exhaustively", [one.name for one in doctor.examine()])

    def test_and_lists_the_checks_by_name(self):
        found = doctor.examine()

        self.assertIn("palette", " ".join(one.detail for one in found))

    def test_a_conformance_module_that_cannot_be_read_is_reported(self):
        def boom():
    ...
    def test_and_one_that_lists_nothing_is_a_failure(self):
