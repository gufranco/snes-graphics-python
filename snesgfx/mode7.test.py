...
class SignTest(unittest.TestCase):
    def test_a_thirteen_bit_value_below_the_halfway_point_is_itself(self):
        self.assertEqual(mode7.signed13(0x0FFF), 0x0FFF)

    def test_and_above_it_is_negative(self):
        self.assertEqual(mode7.signed13(0x1FFF), -1)

    def test_a_sixteen_bit_value_signs_at_its_own_halfway_point(self):
        self.assertEqual(mode7.signed16(0xFFFF), -1)
        self.assertEqual(mode7.signed16(0x7FFF), 0x7FFF)

    def test_the_matrix_reads_its_entries_as_signed(self):
...
class TransformTest(unittest.TestCase):
    def test_the_identity_matrix_leaves_a_pixel_where_it_was(self):
        field = placed()
...
class WrapTest(unittest.TestCase):
    def test_a_coordinate_inside_the_field_is_left_alone(self):
        self.assertEqual(mode7.wrap(100, 200), (100, 200))

    def test_a_coordinate_past_the_edge_comes_back_round(self):
        self.assertEqual(mode7.wrap(1024, 0), (0, 0))

    def test_a_negative_coordinate_comes_back_from_the_other_side(self):
        self.assertEqual(mode7.wrap(-1, 0), (1023, 0))

    def test_a_coordinate_outside_the_field_can_be_asked_about_instead(self):
        self.assertTrue(mode7.outside(1024, 0))
        self.assertFalse(mode7.outside(1023, 1023))


class LayoutTest(unittest.TestCase):
    def test_the_two_halves_of_the_word_hold_different_things(self):
    ...
    def test_the_two_halves_go_back_together(self):
        vram = bytes([0x11, 0x22, 0x33, 0x44])
    ...
    def test_data_of_odd_length_is_refused(self):
        with self.assertRaises(mode7.Truncated):
            mode7.deinterleave(bytes(3))

    def test_halves_of_different_lengths_are_refused(self):
        with self.assertRaises(mode7.Truncated):
            mode7.interleave(bytes(2), bytes(3))

    def test_a_mode_seven_tile_is_stored_pixel_by_pixel(self):
        pixels = bytes(range(64))

        self.assertEqual(mode7.tile(pixels, 0), list(range(64)))

    def test_a_tile_past_the_end_of_the_data_is_refused(self):
        with self.assertRaises(mode7.Truncated):
            mode7.tile(bytes(64), 1)

    def test_the_tile_a_field_position_lands_in_is_the_one_the_map_names(self):
        names = bytes([7]) + bytes(0x3FFF)

        self.assertEqual(mode7.name_at(names, 3, 4), 7)

    def test_a_position_in_the_second_tile_reads_the_second_entry(self):
        names = bytes([0, 9]) + bytes(0x3FFE)

        self.assertEqual(mode7.name_at(names, 8, 0), 9)

    def test_a_position_on_the_second_row_of_tiles_skips_a_whole_row(self):
...
class ReadingTest(unittest.TestCase):
    def test_a_matrix_prints_as_its_four_entries(self):
        self.assertIn("256", repr(IDENTITY))

    def test_a_field_prints_as_where_it_is_centred(self):
