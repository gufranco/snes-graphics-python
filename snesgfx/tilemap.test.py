...
class ScreenTest(unittest.TestCase):
    def test_the_smallest_screen_is_one_quadrant(self) -> None:
        self.assertEqual(tilemap.screen_size(0), (32, 32))

    def test_a_wide_screen_is_two_side_by_side(self) -> None:
        self.assertEqual(tilemap.screen_size(1), (64, 32))

    def test_a_tall_screen_is_two_stacked(self) -> None:
        self.assertEqual(tilemap.screen_size(2), (32, 64))

    def test_the_largest_is_four(self) -> None:
...
class QuadrantTest(unittest.TestCase):
    def test_a_position_in_the_first_quadrant_is_where_it_says(self) -> None:
        self.assertEqual(tilemap.offset_of(3, 4, size=3), 4 * 32 + 3)

    def test_the_second_quadrant_starts_a_whole_quadrant_along(self) -> None:
        self.assertEqual(tilemap.offset_of(32, 0, size=3), tilemap.QUADRANT_ENTRIES)

    def test_the_third_quadrant_of_a_tall_screen_follows_the_first(self) -> None:
        self.assertEqual(tilemap.offset_of(0, 32, size=2), tilemap.QUADRANT_ENTRIES)

    def test_the_third_quadrant_of_a_full_screen_follows_two(self) -> None:
        self.assertEqual(tilemap.offset_of(0, 32, size=3), 2 * tilemap.QUADRANT_ENTRIES)

    def test_the_fourth_quadrant_follows_three(self) -> None:
        self.assertEqual(tilemap.offset_of(32, 32, size=3), 3 * tilemap.QUADRANT_ENTRIES)

    def test_a_position_past_the_screen_wraps_back_into_it(self) -> None:
        self.assertEqual(tilemap.offset_of(64, 0, size=3), tilemap.offset_of(0, 0, size=3))

    def test_a_narrow_screen_wraps_at_its_own_width(self) -> None:
        self.assertEqual(tilemap.offset_of(32, 0, size=0), tilemap.offset_of(0, 0, size=0))


class MapTest(unittest.TestCase):
    def test_a_map_holds_one_entry_for_every_two_bytes(self) -> None:
        self.assertEqual(len(tilemap.decode(bytes(8))), 4)

    def test_an_entry_is_two_bytes_low_first(self) -> None:
    ...
    def test_an_empty_map_is_a_map(self) -> None:
        self.assertEqual(tilemap.decode(b""), [])

    def test_reading_a_position_reaches_the_entry_the_hardware_would(self) -> None:
...
class ReadingTest(unittest.TestCase):
    def test_an_entry_prints_as_the_tile_and_the_block(self) -> None:
        found = repr(tilemap.Entry(tile=9, block=2))

        self.assertIn("9", found)
        self.assertIn("2", found)

    def test_two_entries_holding_the_same_thing_are_the_same_entry(self) -> None:
    ...
    def test_entries_holding_the_same_thing_collapse_in_a_set(self) -> None:
