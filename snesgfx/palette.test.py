...
class TableTest(unittest.TestCase):
    ...
    def test_the_unused_top_bit_does_not_survive_the_round_trip(self):
        self.assertEqual(palette.encode(palette.decode(bytes([0x00, 0x80]))), bytes([0x00, 0x00]))

    def test_an_empty_table_is_a_table(self):
        self.assertEqual(palette.decode(b""), [])

    def test_clearing_the_unused_bit_refuses_a_table_of_odd_length(self):
        with self.assertRaises(palette.Truncated):
            palette.normalise(bytes(3))


class BlockTest(unittest.TestCase):
    def test_a_four_bit_tile_reaches_sixteen_colours_at_a_time(self):
        self.assertEqual(palette.block_size(4), 16)

    def test_a_two_bit_tile_reaches_four(self):
        self.assertEqual(palette.block_size(2), 4)

    def test_an_eight_bit_tile_reaches_the_whole_table(self):
        self.assertEqual(palette.block_size(8), 256)

    def test_a_block_starts_where_its_number_puts_it(self):
        self.assertEqual(palette.block_start(4, 3), 48)

    def test_the_first_block_starts_at_the_beginning(self):
        self.assertEqual(palette.block_start(2, 0), 0)

    def test_a_depth_with_no_palette_block_is_refused(self):
        with self.assertRaises(palette.OutOfRange):
            palette.block_size(3)

    def test_a_block_number_past_the_end_of_the_table_is_refused(self):
        with self.assertRaises(palette.OutOfRange):
            palette.block_start(4, 16)

    def test_a_depth_with_only_one_block_refuses_a_second(self):
        with self.assertRaises(palette.OutOfRange):
            palette.block_start(8, 1)

    def test_resolving_a_pixel_gives_the_colour_the_hardware_would_show(self):
        table = [(0, 0, 0)] * 256
        table[48 + 5] = (0x10, 0x20, 0x30)

        self.assertEqual(palette.resolve(table, 5, depth=4, block=3), (0x10, 0x20, 0x30))

    def test_a_pixel_too_large_for_its_depth_is_refused(self):
        with self.assertRaises(palette.OutOfRange):
            palette.resolve([(0, 0, 0)] * 256, 16, depth=4, block=0)


class TransparencyTest(unittest.TestCase):
    def test_the_first_colour_of_a_block_is_the_one_that_shows_through(self):
        self.assertTrue(palette.is_transparent(0))

    def test_and_no_other_colour_is(self):
